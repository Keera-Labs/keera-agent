...
async def _inject_when_ready(session_id: str, message: str, timeout: float = 30.0) -> None:
    ...
    terminal_manager.write(session_id, f"{message}\r")
...
async def _spawn_headless_agent(agent, project, cwd: str, initial_message: str) -> None:
    ...
    terminal.write(f'{agent.to_command(relay_instructions)}\n'.encode())
    ...
    ready_event.set()
    terminal_manager.write(session_id, f"{initial_message}\r")
