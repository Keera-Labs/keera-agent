...
_migration = importlib.import_module("databases.migrations.2026_06_20_100000_reset_pm_plan_mode")
ResetPmPlanMode = _migration.ResetPmPlanMode


async def _make_template(
    *, name: str, agent_type: str, is_builtin: bool, plan_mode: bool
) -> AgentTemplate:
...
class TestResetPmPlanModeMigration(TestCase, DatabaseTransaction):
    ...
    async def test_leaves_non_pm_plan_mode_untouched(self):
        tpl = await _make_template(
            name="Planner-mig", agent_type="reviewer", is_builtin=True, plan_mode=True
        )
