...
class TestCreateTaskTool(TestCase, DatabaseTransaction):
    ...
    async def test_body_is_stored_correctly(self):
        ...
        self.assertFalse(
            hasattr(task, "description") and task.description is not None,
...
class TestListTasksTool(TestCase, DatabaseTransaction):
    ...
    async def test_filter_by_status(self):
        await TaskFactory.new().create(
            project_id=self.project.id, title="Pending one", status="pending"
        )
        await TaskFactory.new().create(
            project_id=self.project.id, title="Done one", status="completed"
        )
...
class TestSendMessageToolValidation(TestCase, DatabaseTransaction):
    ...
    async def test_receiver_by_name_case_insensitive(self):
        ...
        await AgentFactory.new().create(project_id=project.id, name="PM Agent")
