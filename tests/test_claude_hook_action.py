...
class TestClaudeHookAction(TestCase):
    ...
    async def test_writes_stop_hook_url_from_base_url(self):
    ...
    async def test_defaults_base_url_from_config(self):
        ClaudeHookAction.prepare(self.dir).execute()
        urls = [h["url"] for h in self._stop_hooks(self._read())]
        self.assertIn(f"{Config.get('fastapi.app_url')}/api/claude-stopped", urls)
